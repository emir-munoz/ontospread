...
If some paths of activation converge to the same node, see Fig.~\ref{fig:prize-sa}, 
and the source nodes are different then this node should be 
...
ONTOSPREAD framework\footnote{\url{http://code.google.com/p/ontospread/}}, see Fig.~\ref{fig:diagramas/genera}
...
the algorithm is captured in a separate class (\textit{OntoSpreadState}) that make possible to serialize the 
...
framework is independent from the modeling language of the semantic network (OWL and RDF vocabularies are now supported).
